...
# 8. Delivery is event-based local true-peak safety only.
# 9. No limiter-envelope / no release-tail handbrake / no global ceiling push.
# 10. No RMS makeup inside delivery.
# 11. No global final safety trim as a normal tool.
# 12. If delivery repair goes over budget, executor reports upstream retry need.
# ============================================================
...
def _is_delivery_context(stage_name: str | None, op: Any) -> bool:
    ...
    return (
        ...
        or op_kind in {"true_peak_limiter", "ceiling_trim", "final_balance_guard"}
    )
...
# ============================================================
# EVENT-BASED LOCAL TRUE PEAK SAFETY SHAPER
# ============================================================
# Это НЕ limiter-envelope.
# Это terminal local peak repair:
# - oversample
# - find only real true-peak overs
# - group local events
# - repair only event windows
# - no release tail
# - no global gain envelope
# - no loudness purchase
# ============================================================
...
def _contiguous_regions(mask: np.ndarray) -> list[tuple[int, int]]:
    ...
    for i in range(0, len(changes), 2):
        if i + 1 < len(changes):
            regions.append((int(changes[i]), int(changes[i + 1])))

    return regions


def _detect_true_peak_events_os(
    audio_os: np.ndarray,
    sr_os: int,
    ...
    *,
    pre_ms: float = 0.16,
    post_ms: float = 0.34,
    merge_gap_ms: float = 0.18,
    max_event_ms: float = 6.0,
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    audio_os = _ensure_2d_audio(audio_os)

    target_lin = _db_to_lin(float(target_ceiling_db))
    peak_trace = np.max(np.abs(audio_os), axis=1).astype(np.float32)

    over_mask = peak_trace > target_lin
    raw_regions = _contiguous_regions(over_mask)

    pre_samples = max(1, int(sr_os * float(pre_ms) * 0.001))
    post_samples = max(1, int(sr_os * float(post_ms) * 0.001))
    merge_gap_samples = max(0, int(sr_os * float(merge_gap_ms) * 0.001))

    expanded: list[dict[str, Any]] = []

    for core_start, core_end in raw_regions:
        event_start = max(0, core_start - pre_samples)
        event_end = min(len(peak_trace), core_end + post_samples)

        core_peak = float(np.max(peak_trace[core_start:core_end])) if core_end > core_start else 0.0
        overshoot_db = max(0.0, _lin_to_db(core_peak / max(target_lin, 1e-12)))
    ...
    for ev in expanded:
        ...
        if ev["start"] <= prev["end"] + merge_gap_samples:
            prev["end"] = max(prev["end"], ev["end"])
            prev["core_start"] = min(prev["core_start"], ev["core_start"])
            prev["core_end"] = max(prev["core_end"], ev["core_end"])
            prev["core_peak_lin"] = max(prev["core_peak_lin"], ev["core_peak_lin"])
            prev["overshoot_db"] = max(prev["overshoot_db"], ev["overshoot_db"])
        else:
            merged.append(ev)

    longest_event_ms = 0.0
    total_event_window_samples = 0
    long_event_count = 0

    for ev in merged:
        event_len = max(0, int(ev["end"]) - int(ev["start"]))
        event_ms = event_len / float(sr_os) * 1000.0
        ev["duration_ms"] = float(event_ms)

        longest_event_ms = max(longest_event_ms, event_ms)
        total_event_window_samples += event_len

        if event_ms > float(max_event_ms):
            ev["over_event_duration_budget"] = True
            long_event_count += 1
        else:
            ev["over_event_duration_budget"] = False

    over_sample_count = int(np.sum(over_mask))
    over_sample_ratio = float(over_sample_count / max(len(peak_trace), 1))

    debug = {
        "peak_event_count": int(len(merged)),
        "raw_peak_region_count": int(len(raw_regions)),
        "over_sample_count": int(over_sample_count),
        "over_sample_ratio": round(over_sample_ratio, 8),
        "real_over_ceiling_ratio": round(over_sample_ratio, 8),
        "max_event_overshoot_db": round(
            max([float(ev.get("overshoot_db", 0.0)) for ev in merged], default=0.0),
            4,
        ),
        "longest_event_window_ms": round(float(longest_event_ms), 4),
        "total_event_window_ms": round(
            total_event_window_samples / float(sr_os) * 1000.0,
            4,
        ),
        "long_event_count": int(long_event_count),
        "event_detector": "real_overs_only_no_release_tail",
    }

    return merged, debug


def _raised_cosine_down(n: int, start_gain: float, end_gain: float) -> np.ndarray:
    n = max(int(n), 0)
    ...
    if n == 1:
        return np.array([float(end_gain)], dtype=np.float32)

    t = np.linspace(0.0, 1.0, n, dtype=np.float32)
    curve = 0.5 - 0.5 * np.cos(np.pi * t)

    return (float(start_gain) + (float(end_gain) - float(start_gain)) * curve).astype(np.float32)


def _build_event_gain_curve(
    audio_os: np.ndarray,
    sr_os: int,
    ...
    sample_count = int(audio_os.shape[0])
    target_lin = _db_to_lin(float(target_ceiling_db))

    gain_curve = np.ones(sample_count, dtype=np.float32)
    max_event_gain_reduction_db = 0.0

    for ev in events:
        start = int(ev["start"])
        end = int(ev["end"])
        core_start = int(ev["core_start"])
        core_end = int(ev["core_end"])

        if end <= start:
            continue

        core_peak = float(ev.get("core_peak_lin", 0.0) or 0.0)

        if core_peak <= target_lin:
            continue

        event_gain = target_lin / max(core_peak, 1e-12)
        event_gain = _clamp(event_gain, 0.20, 1.0)

        event_reduction_db = -_lin_to_db(event_gain) if event_gain < 1.0 else 0.0
        max_event_gain_reduction_db = max(max_event_gain_reduction_db, event_reduction_db)

        local = np.ones(end - start, dtype=np.float32)

        rel_core_start = int(_clamp(core_start - start, 0, end - start))
        rel_core_end = int(_clamp(core_end - start, 0, end - start))

        if rel_core_start > 0:
            local[:rel_core_start] = _raised_cosine_down(
                rel_core_start,
                start_gain=1.0,
                end_gain=event_gain,
            )

        if rel_core_end > rel_core_start:
            local[rel_core_start:rel_core_end] = float(event_gain)

        if rel_core_end < len(local):
            local[rel_core_end:] = _raised_cosine_down(
                len(local) - rel_core_end,
                start_gain=event_gain,
                end_gain=1.0,
            )

        gain_curve[start:end] = np.minimum(gain_curve[start:end], local)

    changed_mask = gain_curve < 0.999999
    changed_sample_count = int(np.sum(changed_mask))
    changed_sample_ratio = float(changed_sample_count / max(sample_count, 1))

    debug = {
        "changed_sample_count": int(changed_sample_count),
        "changed_sample_ratio": round(changed_sample_ratio, 8),
        "active_gain_reduction_ratio": round(changed_sample_ratio, 8),
        "max_event_gain_reduction_db": round(float(max_event_gain_reduction_db), 4),
        "max_gain_reduction_db": round(float(max_event_gain_reduction_db), 4),
        "gain_curve_type": "local_event_windows_only",
        "global_release_tail": False,
        "fullband_limiter_envelope": False,
    }

    return gain_curve.astype(np.float32), debug


def _evaluate_local_peak_repair_budget(debug: dict[str, Any]) -> dict[str, Any]:
    over_ratio = float(debug.get("over_sample_ratio", 0.0) or 0.0)
    changed_ratio = float(debug.get("changed_sample_ratio", 0.0) or 0.0)
    changed_to_over = changed_ratio / max(over_ratio, 1e-12)

    max_gr = abs(float(debug.get("max_event_gain_reduction_db", 0.0) or 0.0))
    longest_event_ms = float(debug.get("longest_event_window_ms", 0.0) or 0.0)
    long_event_count = int(debug.get("long_event_count", 0) or 0)

    rms_delta = float(debug.get("rms_delta_db", 0.0) or 0.0)
    residual_tp = debug.get("candidate_after_true_peak_dbtp", debug.get("after_true_peak_dbtp"))
    target_ceiling = debug.get("target_ceiling_db")

    residual_tp_f = float(residual_tp) if residual_tp is not None else None
    target_f = float(target_ceiling) if target_ceiling is not None else None

    warnings: list[str] = []
    failures: list[str] = []

    if changed_ratio > 0.030:
        failures.append("local_peak_repair_changed_too_much_material")
    elif changed_ratio > 0.012:
        warnings.append("local_peak_repair_changed_material_warning")

    if changed_to_over > 36.0 and changed_ratio > 0.005:
        failures.append("changed_to_over_ratio_too_high_not_local_enough")
    elif changed_to_over > 20.0 and changed_ratio > 0.003:
        warnings.append("changed_to_over_ratio_warning")

    if max_gr > 3.0:
        failures.append("local_peak_repair_too_deep")
    elif max_gr > 1.8:
        warnings.append("local_peak_repair_depth_warning")

    if longest_event_ms > 8.0 or long_event_count > 0:
        failures.append("peak_event_too_long_for_terminal_repair")
    elif longest_event_ms > 5.0:
        warnings.append("peak_event_duration_warning")

    if abs(min(rms_delta, 0.0)) > 0.06:
        failures.append("local_peak_repair_rms_loss_over_budget")
    elif abs(min(rms_delta, 0.0)) > 0.025:
        warnings.append("local_peak_repair_rms_loss_warning")

    if residual_tp_f is not None and target_f is not None:
        if residual_tp_f > target_f + 0.06:
            failures.append("residual_true_peak_above_target_after_local_repair")
        elif residual_tp_f > target_f + 0.02:
            warnings.append("residual_true_peak_close_to_target_after_local_repair")
    ...
    if delivery_over_budget:
        retry_hints.extend(
            [
                ...
                "do_not_use_global_safety_trim",
                "reduce_upstream_peak_pressure",
                "consider_peak_aware_rebuild_before_delivery",
                "audition_pre_delivery_before_cutting_support_projection_spark",
                "rerender_musical_graph_before_delivery",
            ]
        )

    return {
        "changed_to_over_ratio": round(float(changed_to_over), 4),
        "limiter_global_handbrake": False,
        "limiter_point_catch": not delivery_over_budget,
        "delivery_damage_budget": {
            "changed_sample_ratio": round(float(changed_ratio), 8),
            "over_sample_ratio": round(float(over_ratio), 8),
            "changed_to_over_ratio": round(float(changed_to_over), 4),
            "max_event_gain_reduction_db": round(float(max_gr), 4),
            "longest_event_window_ms": round(float(longest_event_ms), 4),
            "rms_loss_abs_db": round(abs(min(rms_delta, 0.0)), 4),
            "warnings": warnings,
            "failures": failures,
...
def _apply_local_true_peak_safety_shaper_audio(
    audio: np.ndarray,
    sr: int,
    ceiling_db: float,
    *,
    ...
    before_crest_db = before_peak_db - before_rms_db

    current = dry.copy()

    cumulative_changed_mask: np.ndarray | None = None
    pass_reports: list[dict[str, Any]] = []

    total_peak_event_count = 0
    max_event_overshoot_db = 0.0
    max_event_gain_reduction_db = 0.0
    longest_event_window_ms = 0.0
    total_event_window_ms = 0.0
    over_sample_count_total = 0
    over_sample_ratio_max = 0.0
    ...
    for pass_index in range(max(1, int(max_passes))):
        ...
        events, detect_debug = _detect_true_peak_events_os(
            audio_os=audio_os,
            sr_os=sr_os,
            target_ceiling_db=target_ceiling_db,
        )

        total_peak_event_count += int(detect_debug.get("peak_event_count", 0) or 0)
        over_sample_count_total += int(detect_debug.get("over_sample_count", 0) or 0)
        over_sample_ratio_max = max(
            over_sample_ratio_max,
            float(detect_debug.get("over_sample_ratio", 0.0) or 0.0),
        )
        max_event_overshoot_db = max(
            max_event_overshoot_db,
            float(detect_debug.get("max_event_overshoot_db", 0.0) or 0.0),
        )
        longest_event_window_ms = max(
            longest_event_window_ms,
            float(detect_debug.get("longest_event_window_ms", 0.0) or 0.0),
        )
        total_event_window_ms += float(detect_debug.get("total_event_window_ms", 0.0) or 0.0)
        ...
        gain_curve, gain_debug = _build_event_gain_curve(
            audio_os=audio_os,
            sr_os=sr_os,
            events=events,
            target_ceiling_db=target_ceiling_db,
        ...
        if cumulative_changed_mask is None or len(cumulative_changed_mask) != len(changed_mask):
            cumulative_changed_mask = changed_mask.copy()
        else:
            cumulative_changed_mask = np.logical_or(cumulative_changed_mask, changed_mask)

        max_event_gain_reduction_db = max(
            max_event_gain_reduction_db,
            float(gain_debug.get("max_event_gain_reduction_db", 0.0) or 0.0),
        )

        candidate_os = audio_os * gain_curve[:, None]

        candidate = _downsample_audio(
            audio_os=candidate_os,
            target_sr=sr_os,
            original_sr=sr,
    ...
    candidate_rms_db = _rms_dbfs(candidate)
    candidate_peak_db = _sample_peak_dbfs(candidate)
    candidate_tp_db = _estimate_approx_true_peak_dbtp(
        candidate,
        sr,
        oversample=max(int(oversample), 1),
    )
    candidate_crest_db = candidate_peak_db - candidate_rms_db

    if cumulative_changed_mask is None:
        changed_sample_count = 0
        changed_sample_ratio = 0.0
    else:
        changed_sample_count = int(np.sum(cumulative_changed_mask))
        changed_sample_ratio = float(changed_sample_count / max(len(cumulative_changed_mask), 1))

    debug = {
        "limiter_mode": "event_based_local_true_peak_safety_shaper",
        "legacy_limiter_envelope_removed": True,
        "target_gain_attack_release_removed": True,
        "no_rms_makeup": True,
        "no_creative_compression": True,
        "no_pre_ceiling_smoothing": True,
        "no_global_final_safety_trim": True,
        "touches_only_local_peak_event_windows": True,
        ...
        "before_crest_db": round(before_crest_db, 4),

        "candidate_after_sample_peak_dbfs": round(candidate_peak_db, 4),
        "candidate_after_true_peak_dbtp": round(candidate_tp_db, 4),
        "candidate_after_rms_dbfs": round(candidate_rms_db, 4),
        "candidate_after_crest_db": round(candidate_crest_db, 4),
        "candidate_rms_delta_db": round(candidate_rms_db - before_rms_db, 4),
        "candidate_crest_delta_db": round(candidate_crest_db - before_crest_db, 4),

        "rms_delta_db": round(candidate_rms_db - before_rms_db, 4),
        "crest_delta_db": round(candidate_crest_db - before_crest_db, 4),
        "lufs_delta_db": None,
        "lufs_delta_source": "not_computed_inside_executor",
        "plr_delta_db": round(candidate_crest_db - before_crest_db, 4),
        "plr_delta_source": "peak_minus_rms_proxy",
        "punch_delta_proxy": round(candidate_crest_db - before_crest_db, 4),

        "peak_event_count": int(total_peak_event_count),
        "raw_peak_region_count": None,
        "over_sample_count": int(over_sample_count_total),
        "over_sample_ratio": round(float(over_sample_ratio_max), 8),
        "real_over_ceiling_ratio": round(float(over_sample_ratio_max), 8),
        "changed_sample_count": int(changed_sample_count),
        "changed_sample_ratio": round(float(changed_sample_ratio), 8),
        "active_gain_reduction_ratio": round(float(changed_sample_ratio), 8),
        "max_event_overshoot_db": round(float(max_event_overshoot_db), 4),
        "max_event_gain_reduction_db": round(float(max_event_gain_reduction_db), 4),
        "max_gain_reduction_db": round(float(max_event_gain_reduction_db), 4),
        "longest_event_window_ms": round(float(longest_event_window_ms), 4),
        "total_event_window_ms": round(float(total_event_window_ms), 4),
        "passes_executed": int(len(pass_reports)),
        "pass_reports": pass_reports,
    }

    debug.update(_evaluate_local_peak_repair_budget(debug))

    delivery_over_budget = bool(debug.get("delivery_over_budget", False))

    if delivery_over_budget:
        debug["delivery_candidate_rejected"] = True
        debug["delivery_not_committed_over_budget"] = True
        debug["committed_audio"] = "dry_pre_delivery_passthrough"
        debug["reason"] = "local_peak_repair_over_budget_or_residual_tp"
        committed = dry.copy()
    else:
        debug["delivery_candidate_rejected"] = False
        debug["delivery_not_committed_over_budget"] = False
        debug["committed_audio"] = "local_peak_repaired_candidate"
        committed = candidate

    committed_rms_db = _rms_dbfs(committed)
    committed_peak_db = _sample_peak_dbfs(committed)
    committed_tp_db = _estimate_approx_true_peak_dbtp(
        committed,
        sr,
        oversample=max(int(oversample), 1),
    )
    committed_crest_db = committed_peak_db - committed_rms_db

    debug["after_sample_peak_dbfs"] = round(committed_peak_db, 4)
    debug["after_true_peak_dbtp"] = round(committed_tp_db, 4)
    debug["residual_true_peak_dbtp"] = round(committed_tp_db, 4)
    debug["after_rms_dbfs"] = round(committed_rms_db, 4)
    debug["after_crest_db"] = round(committed_crest_db, 4)
    debug["committed_true_peak_dbtp"] = round(committed_tp_db, 4)
    debug["committed_rms_delta_db"] = round(committed_rms_db - before_rms_db, 4)
    debug["committed_crest_delta_db"] = round(committed_crest_db - before_crest_db, 4)

    return np.nan_to_num(committed, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32), debug
...
def _apply_true_peak_limiter_audio(
    ...
    *,
    ...
    # Внутри больше НЕТ limiter-envelope.
    # threshold / attack / release / mix намеренно не управляют delivery,
    # чтобы render_plan не мог случайно вернуть старый limiter behavior.
    processed, debug = _apply_local_true_peak_safety_shaper_audio(
        audio=audio,
        sr=sr,
    ...
    debug.update(
        {
            "legacy_params_ignored": True,
            "ignored_threshold_db": round(float(threshold_db), 4) if threshold_db is not None else None,
            "ignored_attack_ms": round(float(attack_ms), 4),
            "ignored_release_ms": round(float(release_ms), 4),
            "ignored_mix": round(float(mix), 4),
        }
    )
...
def _resolve_delivery_output_gain(
    ...
    *,
    ...
    blocked_global_trim = False

    if op_kind in {"ceiling_trim", "final_balance_guard"} or primitive_name in {
        "ceiling_trim",
        "final_balance_guard",
        "safety_ceiling_trim",
    }:
        blocked_global_trim = True

    allow_negative_trim = _truthy(params.get("allow_terminal_negative_trim", False))

    applied_gain_db = requested_gain_db
    positive_gain_blocked = False
    negative_gain_blocked = False

    if DELIVERY_BLOCKS_POSITIVE_GAIN and requested_gain_db > 0.0:
        applied_gain_db = 0.0
        positive_gain_blocked = True

    if blocked_global_trim:
        applied_gain_db = 0.0

    elif requested_gain_db < 0.0 and not allow_negative_trim:
        applied_gain_db = 0.0
        negative_gain_blocked = True

    elif requested_gain_db < 0.0 and allow_negative_trim:
        applied_gain_db = _clamp(requested_gain_db, -0.75, 0.0)

    else:
        applied_gain_db = _clamp(applied_gain_db, 0.0, 0.0)
    ...
    if negative_gain_blocked or blocked_global_trim:
        retry_hints.extend(
            [
                "delivery_global_trim_blocked",
                "do_not_use_delivery_as_global_handbrake",
                "use_local_true_peak_event_repair_only",
            ]
        )
    ...
    debug = {
        ...
        "negative_gain_blocked": negative_gain_blocked,
        "blocked_global_trim": blocked_global_trim,
        "allow_negative_trim": bool(allow_negative_trim),
...
def _execute_true_peak_limiter_file(input_path: str, output_path: str, op: Any) -> dict[str, Any]:
    ...
    return {
        "delivery_contract": "terminal_event_based_local_true_peak_safety_only",
        **limiter_debug,
    }
...
def _record_op_result(
    ...
    if op_result["executed"]:
        ...
        if bool(op_extra.get("positive_gain_blocked", False)):
            report["delivery_positive_gain_blocked"] = True

        if bool(op_extra.get("blocked_global_trim", False)):
            report["delivery_global_trim_blocked"] = True
...
def _execute_plan(render_plan: Any, input_path: str | None = None, dry_run: bool = False) -> dict:
    ...
    report: dict[str, Any] = {
        ...
        "delivery_global_trim_blocked": False,
        ...
        "executor_contract": {
            ...
            "delivery_algorithm": "event_based_local_true_peak_safety_shaper",
        },

        "notes": list(_read(render_plan, "notes", []) or [])
        + [
            "executor_active_v8_event_delivery",
            ...
            "delivery_positive_gain_blocked",
            "delivery_global_trim_blocked_by_default",
            "delivery_cannot_buy_loudness",
            "delivery_event_based_local_true_peak_safety",
            "legacy_limiter_envelope_removed",
            "target_gain_attack_release_removed",
            "delivery_does_not_touch_below_target_ceiling_except_event_windows",
            ...
            "pre_delivery_audition_export_enabled",
        ],
    }
    ...
    pre_delivery_path = node_paths.get("finish_stage_out")
    post_delivery_path = node_paths.get(final_output_node)

    report["pre_delivery_audition_path"] = pre_delivery_path
    report["post_delivery_path"] = post_delivery_path

    report["audition_paths"] = {
        "pre_delivery": pre_delivery_path,
        "post_delivery": post_delivery_path,
    }

    if pre_delivery_path:
        report["notes"].append("pre_delivery_audition_export_available")

    if report.get("delivery_over_budget"):
        report["notes"].append("delivery_over_budget_do_not_cut_support_blindly")
        report["notes"].append("audition_pre_delivery_before_support_projection_spark_changes")
